...
//displays the planet that was selected
function displayPlanet(){
	'use strict';

	//gets the value of planetSelect and its index
	var planet = planetSelect.value;
	var planetPosition = planetSelect.selectedIndex;


	if(planet == 'galaxies'){
		//if galaxies create drop down list and display galaxy selected
		picture.src = '';
		nameInfo.innerHTML = '';
		description.innerHTML = '';
		updateMenu();
		if(getGalaxies()){
			getGalaxies().onchange = displayGalaxy;
		}
	}else if(planet == ''){
		//if select do not display anything
		removeGalaxies();
		picture.src = '';
		nameInfo.innerHTML = '';
		description.innerHTML = planetInfo[planetPosition];
	}else{
		//if a planet is selected display the planet
		removeGalaxies();
		picture.src = 'images/' + planet + '.jpg';
		nameInfo.innerHTML = planetSelect.options[planetPosition].text;
		description.innerHTML = planetInfo[planetPosition];
	}
}

//creates a drop down menu for galaxies and adds it to the web page
function updateMenu(){
	'use strict';

	var select = document.createElement('select');
	select.setAttribute('id', 'galaxies');
	select.setAttribute('name', 'galaxies');
	...
	option2.setAttribute('value', 'andromeda');
	option2.text = 'Andromeda Galaxy';
	select.add(option2);

	var option3 = document.createElement('option');
	option3.setAttribute('value', 'm82');
	option3.text = 'M82 Galaxy';
	select.add(option3);

	var option4 = document.createElement('option');
	option4.setAttribute('value', 'milky');
	option4.text = 'Milky Way';
	select.add(option4);

	var option5 = document.createElement('option');
	option5.setAttribute('value', 'ngc');
	option5.text = 'NGC 5866';
	select.add(option5);

	var option6 = document.createElement('option');
	option6.setAttribute('value', 'omega');
	option6.text = 'Omega Centauri';
	select.add(option6);

	var option7 = document.createElement('option');
	option7.setAttribute('value', 'whirlpool');
	option7.text = 'Whirlpool Galaxy';
	select.add(option7);

	var output = document.getElementById('output');

	field.insertBefore(select, output);
}

//displays the galaxy that was selected
function displayGalaxy(){
	'use strict';
	var galaxySelect = getGalaxies();
	var galaxy = galaxySelect.value;
	var galaxyPosition = galaxySelect.selectedIndex;

	if(galaxy == ''){
		//if select is selected do not display anything
		picture.src = '';
		nameInfo.innerHTML = '';
		description.innerHTML = galaxyInfo[galaxyPosition];
	}else{
		//if a galaxy is selected display the galaxy
		picture.src = 'images/' + galaxy + '.jpg';
		nameInfo.innerHTML = galaxySelect.options[galaxyPosition].text;
		description.innerHTML = galaxyInfo[galaxyPosition];
	}
}

//removes the galaxies drop down list from the web page
function removeGalaxies(){
	'use strict';
	var galaxySelect = getGalaxies();
	if(galaxySelect){
		field.removeChild(galaxySelect);
	}
}

//gets the galaxies drop down list when it's there
function getGalaxies(){
	'use strict';
	return document.getElementById('galaxies');
}

//calls displayPlanet when the planet drop down list is changed
function init(){
	'use strict';
	planetSelect.onchange = displayPlanet;
}
