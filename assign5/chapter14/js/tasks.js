...
window.onload = function(){
	...
	document.getElementById('theForm').onsubmit = function() {
		...
		//Chapter 14 Pursue Bullet 3
        //clear the form value
        task.value = "";
		
		//Chapter 14 Puruse Bullet 4
		// Update the output:
		var message = 'There are now <b>' + tasks.length + 
				'</b> item(s) in the to-do list. Just added:<br>' + t.toString() + '<br><br>';
		message += '<h2>Tasks: </h2><ul>';
		for(var i = 0, count = tasks.length; i < count; i++) {
			message += '<li>' + tasks[i].toString() + '</li>';
		}
		message += '</ol>';
